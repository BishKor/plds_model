...
def jointloglikelihood(y, nsd, nn, nts, mu, covd, Q, Q0, m0, A, u, B, i):
    def f(dC):
        di = dC[0]
        Ci = dC[1:]

        jll = sum(-y[t*nn+ i] * Ci @ mu[t*nld:(t+1)*nld] - y[t*nn+i] * di \
                  + np.exp(Ci @ mu[t*nld:(t+1)*nld] + .5 * (Ci @ covd[t] @ Ci) +
                    di) for t in range(nts-1))
        return jll
    return f


def jllDerivative(nn, nld, mu, covd, covod, nts, y, i):
    def f(dC):
        di = dC[0]
        Ci = dC[1:]

        djlld = sum(-y[t*nn+i] + np.exp(Ci @ mu[t*nld:(t+1)*nld] + di + .5 * (Ci @ covd[t] @ Ci))
                           for t in range(nts-1))

        djllC = sum(-y[t*nn+i] * mu[t*nld:(t+1)*nld] + \
                     np.exp(Ci @ mu[t*nld:(t+1)*nld] + di + .5 * (Ci @ covd[t] @ Ci)) * \
                     (mu[t*nld:(t+1)*nld] + covd[t] @ Ci)
                     for t in range(nts-1))

        return np.concatenate([[djlld], djllC])
    return f


def jllHessian(nn, nld, mu, covd, covod, nts, y, i):
    def f(dC):
        di = dC[0]
        Ci = dC[1:]
        ...
        for i in range(nn):
            block = np.zeros((1 + nld, 1 + nld))
            block[0, 0] = sum(np.exp(Ci @ mu[t*nld:(t+1)*nld] + di + .5 * (Ci @ covd[t] @ Ci)) for t in range(nts-1))

            block[0, 1:] = sum((mu[t*nld:(t+1)*nld] + covd[t] @ Ci) * np.exp(Ci @ mu[t*nld:(t+1)*nld] + di + \
                    .5 * (Ci @ covd[t] @ Ci)) for t in range(nts-1))

            block[1:, 0] = block[0, 1:]

            block[1:, 1:] = sum((covd[t] +
                np.outer(mu[t*nld:(t+1)*nld] + covd[t] @ Ci, (mu[t*nld:(t+1)*nld] + covd[t] @ Ci).T))*\
                np.exp(Ci @ mu[t*nld:(t+1)*nld] + di + .5 * (Ci @ covd[t] @ Ci))
                for t in range(nts-1))
        return block
    return f
...
def runmodel(y, u, nts, nn, nld, nsd):
    ...
    for epoch in range(max_epochs):
        ...
        # each neuron
        for i in range(nn):
            dC = np.concatenate([[d[i]], C[i]])

            dC = nr_algo(jointloglikelihood(y, nsd, nn, nts, mu, covd, q, q0, m0, A, u, B, i),
                         jllDerivative(nn, nld, mu, covd, covod, nts, y, i),
                         jllHessian(nn, nld, mu, covd, covod, nts, y, i),
                         dC, mode='dense')
            d[i] = dC[0]
            C[i] = dC[1:]
